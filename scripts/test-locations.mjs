...
// ── T8/T9: vervallen-filter (directe Anthropic-aanroep, met seed/teardown) ───

console.log('\n\n' + '='.repeat(60));
console.log('T8/T9 — vervallen-filter');
console.log('='.repeat(60));

// Zoek een Rotterdam-locatie om tijdelijk op vervallen te zetten
const { data: rotterdamLocs } = await sb.from('locations').select('id, naam, status').eq('tenant_id', CHASE_STAGING).ilike('naam', '%rotterdam%').limit(1);
const T89_LOCATIE = rotterdamLocs?.[0];

if (!T89_LOCATIE) {
  console.log('SKIP T8/T9 — geen Rotterdam-locatie gevonden in Chase tenant');
} else {
  const T89_ORIG_STATUS = T89_LOCATIE.status ?? null;

  // Seed: zet locatie op vervallen
  const { error: seedErr89 } = await sb.from('locations').update({ status: 'vervallen' }).eq('id', T89_LOCATIE.id);
  if (seedErr89) {
    console.log('SKIP T8/T9 — seeddata mislukt: ' + seedErr89.message);
  } else {
    // T8: vervallen locatie mag NIET verschijnen bij gewone Rotterdam-zoekopdracht
    {
      console.log(`\nT8 — ${T89_LOCATIE.naam} (vervallen) niet in gewone Rotterdam-resultaten`);
      let t8Pass = 0;
      for (let run = 1; run <= RUNS; run++) {
        process.stdout.write(`  Run ${run}/${RUNS}... `);
        let result;
        try {
          result = await Promise.race([
            runOnce('Welke locaties hebben we in Rotterdam?'),
            new Promise((_, reject) => setTimeout(() => reject(new Error('TIMEOUT')), RUN_TIMEOUT_MS)),
          ]);
        } catch (err) {
          console.log('FAIL (timeout/error)');
        ...
        const nameRe = new RegExp(T89_LOCATIE.naam.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
        const failures = [];
        if (!result.calledTools.includes('search_locations')) failures.push('search_locations niet aangeroepen');
        if (nameRe.test(result.finalText)) failures.push(`${T89_LOCATIE.naam} verschijnt ondanks vervallen-status`);
        const passed = failures.length === 0;
        if (passed) t8Pass++;
        console.log(passed ? 'PASS' : 'FAIL');
        failures.forEach(f => console.log('    ✗', f));
      ...
      console.log(`T8: ${t8Pass}/${RUNS} runs geslaagd ${t8Pass === RUNS ? '✓' : '✗'}`);
      if (t8Pass === RUNS) totalPass++; else totalFail++;
    }

    // T9: bij expliciete vraag naar vervallen locaties mag de locatie WEL verschijnen
    {
      console.log(`\nT9 — ${T89_LOCATIE.naam} (vervallen) WEL zichtbaar bij expliciete vervallen-zoekopdracht`);
      let t9Pass = 0;
      for (let run = 1; run <= RUNS; run++) {
        process.stdout.write(`  Run ${run}/${RUNS}... `);
        let result;
        try {
          result = await Promise.race([
            runOnce('Welke locaties gebruiken we niet meer?'),
            new Promise((_, reject) => setTimeout(() => reject(new Error('TIMEOUT')), RUN_TIMEOUT_MS)),
          ]);
        } catch (err) {
          console.log('FAIL (timeout/error)');
        ...
        const failures = [];
        if (!result.calledTools.includes('search_locations')) failures.push('search_locations niet aangeroepen');
        const hasInclVervallen = result.toolInputsParsed.some(inp => inp.inclVervallen === true);
        if (!hasInclVervallen) failures.push('search_locations aangeroepen zonder inclVervallen: true');
        const nameRe = new RegExp(T89_LOCATIE.naam.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i');
        if (!nameRe.test(result.finalText)) failures.push(`${T89_LOCATIE.naam} ontbreekt in antwoord bij expliciete vervallen-query`);
        const passed = failures.length === 0;
        if (passed) t9Pass++;
        console.log(passed ? 'PASS' : 'FAIL');
        failures.forEach(f => console.log('    ✗', f));
      ...
      console.log(`T9: ${t9Pass}/${RUNS} runs geslaagd ${t9Pass === RUNS ? '✓' : '✗'}`);
      if (t9Pass === RUNS) totalPass++; else totalFail++;
    }

    // Teardown: herstel originele status
    const { error: teardownErr89 } = await sb.from('locations').update({ status: T89_ORIG_STATUS }).eq('id', T89_LOCATIE.id);
    if (teardownErr89) console.log('(teardown fout: ' + teardownErr89.message + ')');
  }
}
