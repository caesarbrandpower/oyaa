...
const OPEN_MARKER_RE = /\[(?:UITZOEKEN\s+INTERN|AFSTEMMEN\s+MET\s+KLANT|CIJFERS\s+TOEVOEGEN|CHECK:[^\]]*)\]/gi;
...
export async function ingestDocument({
  ...
  const openMarkersCount = (content.match(OPEN_MARKER_RE) ?? []).length;
  ...
  const { data: doc, error: docError } = await supabase
    ...
    .insert({
      ...
      raw_content: content,
      open_markers_count: openMarkersCount,
    })
    .select('id')
    .single();
  if (docError) {
