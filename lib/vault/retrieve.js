...
export async function retrieveVaultContext({
  ...
  // Met een naam in de vraag: naam-gematchte docs komen door bij ≥ 0.45;
  // docs zonder naam-match moeten ≥ 0.55 scoren om door te komen.
  // Zonder naam in de vraag (brede vragen): semantische fallback bij ≥ 0.50.
  const queryNames = extractQueryNames(trimmed);
  const hasNameQuery = queryNames.length > 0;

  const THRESHOLD_NAMED_MATCH = 0.45;  // naam-gematchte doc mag lager scoren
  const THRESHOLD_NO_NAME_MATCH = 0.55; // doc zonder naam-match moet sterker zijn
  const THRESHOLD_FALLBACK = 0.50;      // brede vraag zonder eigennamen
