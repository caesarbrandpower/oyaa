...
describe('chunkText', () => {
  ...
  it('heeft overlap: het einde van chunk N komt terug aan het begin van chunk N+1', () => {
    ...
    const result = chunkText(text, { maxTokens: 800, overlapTokens: 100 });
    expect(result.length).toBeGreaterThan(1);
    // chunk N+1 begint letterlijk met de laatste overlapChars (100 tokens * 4) van chunk N
    const overlapPrefix = result[0].content.slice(-400);
    expect(result[1].content.startsWith(overlapPrefix)).toBe(true);
  });
