  ...
  for (const raw of content.split('\n')) {
    ...
    if (line.startsWith('|') && line.endsWith('|')) {
      ...
      if (rawCells.length >= 2) {
        ...
        // Gebruik jsPDF's eigen lijnhoogte zodat rowH exact aansluit op de werkelijke rendering
        ...
        const rowH = Math.max(...colLineCounts) * lineH + 6;
        checkPage(rowH);
          ...
          if (ci === 0) {
            ...
            for (const seg of segs) {
              ...
              for (let wi = 0; wi < words.length; wi++) {
                ...
                doc.text(word, cx, y + lineOff * lineH);
                cx += sw;
              }
            }
            doc.setTextColor(0, 0, 0);
