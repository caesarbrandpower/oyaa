...
export async function downloadPdfDoc(content, title, logos = {}, extras = null, filename = null, outputType = null, client = null, project = null) {
  ...
  for (const raw of content.split('\n')) {
    ...
    // Table rows — check raw line first so rows with [LABELS] are also caught
    if (line.startsWith('|') && line.endsWith('|')) {
      const rawCells = line.slice(1, -1).split('|').map(c => c.trim()).filter(c => c && !c.match(/^[-:]+$/));
      if (rawCells.length >= 2) {
        const col1W = maxWidth * 0.38;
        const col2X = margin + col1W + 4;
        const col2W = maxWidth - col1W - 4;
        // col1: strip labels and bold markup, render bold
        const col1Text = rawCells[0].replace(/\*\*/g, '').replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '').trim();
        doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
        const col1Lines = doc.splitTextToSize(col1Text, col1W);
        // col2: may contain [LABELS] — measure line count first
        const col2Raw = rawCells.slice(1).join(' ').replace(/\*\*/g, '');
        const col2Segs = splitSegments(col2Raw);
        let col2LineCount = 1;
        {
          let cx = 0;
          for (const seg of col2Segs) {
            if (!seg.text) continue;
            const words = seg.red ? [seg.text] : seg.text.split(' ').filter(w => w);
            for (let wi = 0; wi < words.length; wi++) {
              ...
              const sw = doc.getTextWidth(word);
              if (cx > 0 && cx + sw > col2W) { col2LineCount++; cx = 0; }
              cx += sw;
            }
          }
        }
        const rowH = Math.max(col1Lines.length, col2LineCount) * 5 + 2;
        checkPage(rowH);
        doc.setFont('helvetica', 'bold'); doc.setFontSize(10);
        doc.text(col1Lines, margin, y);
        // Render col2 inline with label support
        let cx2 = col2X;
        const rowStartY = y;
        let lineOffset = 0;
        for (const seg of col2Segs) {
          ...
          for (let wi = 0; wi < words.length; wi++) {
            const word = wi < words.length - 1 ? words[wi] + ' ' : words[wi];
            const sw = doc.getTextWidth(word);
            if (cx2 > col2X && cx2 + sw > col2X + col2W) { lineOffset++; cx2 = col2X; }
            doc.text(word, cx2, rowStartY + lineOffset * 5);
            cx2 += sw;
          }
        }
        doc.setTextColor(0, 0, 0);
