...
export function buildChaseTitle(outputType, client, project) {
  ...
  switch (outputType) {
    ...
    case 'account-to-pm':
      return cp ? `BRIEFING ACCOUNT → PM — ${cp}` : 'BRIEFING ACCOUNT → PM';
    case 'account-to-creation':
      return cp ? `BRIEFING ACCOUNT → CREATIE — ${cp}` : 'BRIEFING ACCOUNT → CREATIE';
...
function buildChaseTitleForPdf(outputType, client, project) {
  ...
  if (!t) return null;
  return t.replace(/→/g, '-').replace(/×/g, 'x').replace(/—/g, '-').replace(/–/g, '-');
}
      ...
      for (let pi = 0; pi < extras.photos.length; pi += 2) {
        ...
        for (let col = 0; col < 2; col++) {
          ...
          if (photo?.name) {
            ...
            const x = margin + col * (photoW + gap);
            doc.text(photo.name.slice(0, 30), x, y);
            doc.setTextColor(0, 0, 0);
          }
        }
        if (p1?.name || p2?.name) y += 5;
      }
      y += 3;
    }
...
export async function downloadWordDoc(content, title, logos = {}, extras = null, filename = null, outputType = null, client = null, project = null) {
  ...
  if (logos.chaseBuffer || logos.clientBuffer) {
    const makeLogoCell = (buffer, align) => {
      ...
      return new docx.TableCell({
        ...
        shading: { fill: '111111', type: docx.ShadingType.SOLID },
        borders: {
  ...
  paragraphs.push(new docx.Paragraph({
    children: [new docx.TextRun({ text: wordTitle, bold: true, size: 28 })],
    spacing: { after: 320 },
  ...
  for (const raw of content.split('\n')) {
    ...
    if (boldSection) {
      ...
      if (rest) runs.push(new docx.TextRun({ text: ': ' + rest.replace(/\*\*/g, ''), size: 22 }));
