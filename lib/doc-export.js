...
export function buildFilename(outputType, client, project, extension) {
  ...
  // Vermijd "Coca-Cola Coca-Cola HHZH" als project de client als prefix bevat
  let cpPart;
  if (client && project) {
    const cSan = sanitizePart(client);
    const pSan = sanitizePart(project);
    if (pSan.toLowerCase().startsWith(cSan.toLowerCase())) {
      // "Coca-Cola HHZH".startsWith("Coca-Cola") → gebruik project direct
      cpPart = pSan;
    } else {
      cpPart = [cSan, pSan].filter(Boolean).join(' ');
    }
  } else {
    cpPart = [client, project].filter(Boolean).map(sanitizePart).filter(Boolean).join(' ');
  }
  ...
  if (logos.chaseBase64 || effectiveClientBase64) {
    ...
    if (effectiveClientBase64) {
      try {
        ...
        const w = Math.min((dims.w / dims.h) * LOGO_H, 60);
        doc.addImage(effectiveClientBase64, getImageFormat(effectiveClientBase64), pageWidth - margin - w, 4, w, LOGO_H);
      } catch { /* skip */ }
    }
    y = LOGO_H + 20;
  }
