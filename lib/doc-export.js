...
// ── Allday phase compact renderer ─────────────────────────────────────────────

const ALLDAY_PHASE_FULL = [
...
// Transforms the raw numbered-list Projectfase block to compact two-line format
// used in Word and PDF export.
function renderPhaseCompact(content) {
  const m = content.match(/(\*\*Projectfase\*\*\n\nFase (\d+) van 10[^\n]*\n\n)((?:\d+\.[^\n]+\n?){1,10})/);
  if (!m) return content;
  const phaseNum = parseInt(m[2]);
  const name = ALLDAY_PHASE_FULL[phaseNum - 1] || `Fase ${phaseNum}`;
  const desc = ALLDAY_PHASE_DESC[phaseNum - 1] || '';
  const nextNum = phaseNum < 10 ? phaseNum + 1 : null;
  const nextName = nextNum ? (ALLDAY_PHASE_FULL[nextNum - 1] || `Fase ${nextNum}`) : null;
  const nextDesc = nextNum ? (ALLDAY_NEXT_DESC[phaseNum - 1] || '') : null;

  let compact = `**Projectfase**\n\n**► Fase ${phaseNum} van 10 — ${name}**\n${desc}`;
  if (nextName) compact += `\n\n→ Volgende stap: Fase ${nextNum} — ${nextName}\n${nextDesc}`;
  return content.replace(m[0], compact + '\n');
}
...
const FILENAME_TYPE_LABELS = {
  ...
  'recording': 'Meeting transcript',
  'allday-gespreksverslag': 'Gespreksverslag',
  'allday-debrief': 'Debrief',
};

// outputType: the document type key (e.g. 'account-to-pm')
export function buildFilename(outputType, client, project, extension, createdAt = null) {
  function sanitizePart(s) {
    return s.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '').replace(/[→×—–]/g, '-').replace(/[<>:"/\\|?*]/g, '').trim();
  }
  // Opname-threads krijgen eigen bestandsnaamformaat: "[client] — Meeting transcript [datum]"
  if (outputType === 'recording') {
    const sanitizedClient = client ? sanitizePart(client) : null;
    let datePart = '';
    if (createdAt) {
    ...
    const label = ['Meeting transcript', datePart].filter(Boolean).join(' ');
    const parts = [sanitizedClient, label].filter(Boolean);
    return (parts.join(' — ').slice(0, 80) || 'Meeting transcript') + '.' + extension;
  }
  const typeLabel = FILENAME_TYPE_LABELS[outputType] || null;
  // All Day types: [Type] - [Client] - [YYYY-MM-DD]
  if (outputType === 'allday-gespreksverslag' || outputType === 'allday-debrief') {
    const d = createdAt ? new Date(createdAt) : new Date();
    const datePart = d.toISOString().slice(0, 10);
    const sanitizedClient = client ? sanitizePart(client) : null;
    const parts = [sanitizedClient, typeLabel, datePart].filter(Boolean);
    return (parts.join(' - ').slice(0, 80) || 'document') + '.' + extension;
  }
...
export function buildChaseTitle(outputType, client, project) {
  ...
  switch (outputType) {
    case 'field-briefing':
      return cp ? `AMBASSADORSBRIEFING — ${cp}` : 'AMBASSADORSBRIEFING';
    case 'account-to-pm':
    ...
    case 'external-debrief':
      return cp ? `EVALUATIE — ${cp}` : 'EVALUATIE';
    case 'allday-gespreksverslag':
      return cp ? `Gespreksverslag — ${cp}` : 'Gespreksverslag';
    case 'allday-debrief':
      return cp ? `Debrief — ${cp}` : 'Debrief';
    case 'recording':
      return client ? `Meeting transcript — ${client}` : 'Meeting transcript';
    default:
      return null;
  }
}
...
async function createHeaderImageBuffer(chaseBuffer, clientBuffer) {
  ...
  async function drawLogo(buf, leftEdge, rightEdge, alignRight) {
    ...
    const head = new TextDecoder().decode(buf.slice(0, 100));
    const isSvg = head.includes('<svg') || head.includes('<?xml');
    let drawBuf = buf;
    if (isSvg) {
      const svgBlob = new Blob([buf], { type: 'image/svg+xml' });
      const svgDataUrl = await new Promise((r) => {
        const fr = new FileReader(); fr.onload = () => r(fr.result); fr.onerror = () => r(null); fr.readAsDataURL(svgBlob);
      });
      if (!svgDataUrl) return;
      const pngDataUrl = await svgDataUrlToPng(svgDataUrl);
      if (!pngDataUrl) return;
      const b64 = pngDataUrl.split(',')[1];
      const bin = atob(b64);
      drawBuf = new Uint8Array(bin.length);
      for (let i = 0; i < bin.length; i++) drawBuf[i] = bin.charCodeAt(i);
    }
    const blob = new Blob([drawBuf], { type: 'image/png' });
...
function buildDocTypeLabel(outputType) {
  switch (outputType) {
    case 'field-briefing': return 'AMBASSADORSBRIEFING';
    ...
    case 'recording': return 'Meeting transcript';
    case 'allday-gespreksverslag': return 'Gespreksverslag';
    case 'allday-debrief': return 'Debrief';
    default: return null;
  }
}
...
export async function downloadPdfDoc(content, title, logos = {}, extras = null, filename = null, outputType = null, client = null, project = null) {
  ...
  const BODY_LH = 5.5;    // body regelafstand mm
  const HEAD_BEFORE = 7;  // ruimte boven kop mm
  const HEAD_AFTER = 3;   // ruimte onder kop mm
  const PARA_GAP = 4;     // ruimte na alinea mm
  const SECT_GAP = 6;     // ruimte na sectie mm
  const BULL_INDENT = 8;  // bullet inspringing mm vanaf margin
  const BULL_GAP = 3;     // ruimte na bullet mm
  const CELL_LH = 5.5;    // tabelcel regelafstand mm
  const PAGE_MAX = 276;   // ondergrens pagina mm

  let atPageTop = false;

  function ensurePage(needed) {
    if (y + needed > PAGE_MAX) {
      doc.addPage();
      y = margin;
      atPageTop = true;
      return true;
  ...
  function addSpace(mm) {
    if (!atPageTop) y += mm;
  }

  // Measure height of inline segments without rendering
  function measureSegs(segs, startX, maxX) {
    doc.setFontSize(10);
    let cx = startX; let lines = 1;
    for (const seg of segs) {
      ...
      for (let wi = 0; wi < words.length; wi++) {
        ...
        if (cx > startX && cx + sw > maxX) { lines++; cx = startX; }
        cx += sw;
      }
    }
  ...
  // Render inline segments at current y, advancing y on line-wrap
  function renderSegs(segs, startX, maxX) {
    doc.setFontSize(10);
    let cx = startX;
    for (const seg of segs) {
      if (!seg.text) continue;
      ...
      for (let wi = 0; wi < words.length; wi++) {
        const word = wi < words.length - 1 ? words[wi] + ' ' : words[wi];
        const sw = doc.getTextWidth(word);
        if (cx > startX && cx + sw > maxX) { y += BODY_LH; cx = startX; }
        doc.text(word, cx, y);
        cx += sw;
      }
      // Preserve trailing space so the next segment starts with proper separation
      if (!seg.red && seg.text.endsWith(' ') && words.length > 0) {
        cx += doc.getTextWidth(' ');
      }
    }
  ...
  ensurePage(wrappedTitle.length * 9 + 16);
  ...
  const isAlldayTypePdf = outputType === 'allday-gespreksverslag' || outputType === 'allday-debrief';

  // Convert numbered phase list to compact two-line format for allday documents
  if (isAlldayTypePdf) content = renderPhaseCompact(content);

  function renderHeading(text, fontSize, lh, cleanFn = t => t) {
    doc.setFont('helvetica', 'bold'); doc.setFontSize(fontSize);
    const clean = cleanFn(text).replace(/[→×—–]/g, '-').trim();
    const wrapped = doc.splitTextToSize(clean, maxWidth);
    const totalH = wrapped.length * lh;
    // Orphan protection: kop + HEAD_AFTER + 2 regels body moeten passen
    addSpace(HEAD_BEFORE);
    ensurePage(totalH + HEAD_AFTER + 2 * BODY_LH);
  ...
  for (const raw of content.split('\n')) {
    const line = raw.trim();
    if (line === '') { addSpace(PARA_GAP); continue; }
    if (/^-{3,}$/.test(line) || /^\*{3,}$/.test(line)) { addSpace(SECT_GAP); continue; }

    // ── H1
    const h1 = line.match(/^#\s+(.+)$/);
    if (h1) {
      if (skipFirstHeading) { skipFirstHeading = false; continue; }
      renderHeading(h1[1], 14, 7, t => t.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, ''));
      continue;
    }
    // ── H2
    ...
    if (h2) {
      ...
      renderHeading(h2[1].replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '').toUpperCase(), 12, 6.5);
      continue;
    }
    // ── H3
    ...
    if (h3) {
      skipFirstHeading = false;
      renderHeading(h3[1], 11, 6, t => t.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, ''));
      continue;
    }

    // ── Bold section heading: **Label** rest
    ...
    if (boldSection) {
      ...
      if (skipFirstHeading) {
        ...
        if (boldNorm === titleNorm || !rest) { skipFirstHeading = false; continue; }
      }
      skipFirstHeading = false;
      const labelClean = boldText.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '').trim();
      if (rest) {
        // **Label**: inline body text — render as bold label + normal rest
        const fullSegs = [
          { text: labelClean + ': ', bold: true, red: false },
          ...splitSegments(rest.replace(/\*\*/g, '')),
        ];
        const h = measureSegs(fullSegs, margin, pageWidth - margin);
        addSpace(HEAD_BEFORE);
        ensurePage(h + HEAD_AFTER + 2 * BODY_LH);
        renderSegs(fullSegs, margin, pageWidth - margin);
        y += BODY_LH; // advance past last rendered line
        addSpace(HEAD_AFTER);
      } else {
        // Standalone bold label — treat as heading
        doc.setFont('helvetica', 'bold'); doc.setFontSize(11);
        const wrapped = doc.splitTextToSize(labelClean, maxWidth);
        const totalH = wrapped.length * 6;
        addSpace(HEAD_BEFORE);
        ensurePage(totalH + HEAD_AFTER + 2 * BODY_LH);
        doc.text(wrapped, margin, y);
        y += totalH;
    ...
    if (bulletMatch || numberedMatch) {
      ...
      const bIndentX = margin + BULL_INDENT;
      const segs = splitSegments(itemText.replace(/\*\*/g, ''));
      const h = measureSegs(segs, bIndentX, pageWidth - margin);
      ensurePage(BODY_LH + h + BULL_GAP);
      doc.text(prefix, margin, y);
      renderSegs(segs, bIndentX, pageWidth - margin);
      y += BODY_LH;
      addSpace(BULL_GAP);
      continue;
    }
    ...
    if (line.startsWith('|') && line.endsWith('|')) {
      ...
      if (rawCells.length >= 2) {
        ...
        // Pre-measure all columns — max lines determines row height
        const colLineCounts = rawCells.map((cell, ci) => {
          const cellText = cell.replace(/\*\*/g, '');
          doc.setFont('helvetica', ci === 0 ? 'bold' : 'normal'); doc.setFontSize(10);
          const plain = cellText.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '...').trim() || ' ';
          if (ci === 0) return doc.splitTextToSize(plain, colW).length;
          // Simulate word-wrap for value cells
        ...
        const rowH = Math.max(...colLineCounts) * CELL_LH + 3;
        // Tabelrij nooit splitsen over pagina's
        ensurePage(rowH);
        rawCells.forEach((cell, ci) => {
          ...
          if (ci === 0) {
            ...
            doc.text(doc.splitTextToSize(cellText.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '').trim(), colW), colX, y);
          } else {
            const segs = splitSegments(cellText);
            ...
            for (const seg of segs) {
              ...
              for (let wi = 0; wi < words.length; wi++) {
                ...
                doc.text(word, cx, y + lineOff * CELL_LH);
                cx += sw;
              }
            }
            doc.setTextColor(0, 0, 0);
        ...
        ensurePage(wrapped.length * BODY_LH + PARA_GAP);
        doc.text(wrapped, margin, y);
        y += wrapped.length * BODY_LH;
        addSpace(PARA_GAP);
        atPageTop = false;
      }
      continue;
    ...
    // ── Regular body text (with optional inline labels/bold)
    const pdfParaText = (isAlldayTypePdf && !line.startsWith('*') && !line.startsWith('#') && !line.startsWith('|'))
      ? line.replace(/^([A-Za-z][^:\n*]*):([^\s:/])/, '$1: $2')
      : line;
    const segs = splitSegments(pdfParaText.replace(/\*\*/g, ''));
    const bodyText = segs.map(s => s.text).join('').trim();
    if (!bodyText) { addSpace(PARA_GAP); continue; }
    const h = measureSegs(segs, margin, pageWidth - margin);
    ensurePage(h + PARA_GAP);
    renderSegs(segs, margin, pageWidth - margin);
    y += BODY_LH; // advance past last rendered line
    addSpace(PARA_GAP);
  }
...
export async function buildWordBlob(content, title, logos = {}, extras = null, outputType = null, client = null, project = null) {
  ...
  try {
    const headerBuf = await createHeaderImageBuffer(logos.chaseBuffer ?? null, effectiveClientBuffer ?? null);
    if (headerBuf) {
  ...
  // All Day: gecombineerde H1 "Gespreksverslag — Heineken", geen aparte subtitle
  let wordMainTitle, wordCpLabel;
  if (isAlldayType) {
    const base = wordTypeLabel || (outputType === 'allday-debrief' ? 'Debrief' : 'Gespreksverslag');
    wordMainTitle = client ? `${base} — ${client}` : base;
    wordCpLabel = null;
  } else {
    wordMainTitle = wordTypeLabel || title.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '').trim();
    wordCpLabel = [client, project].filter(Boolean).join(' ');
  }
  ...
  // Convert numbered phase list to compact two-line format for allday documents
  if (isAlldayType) content = renderPhaseCompact(content);
  ...
  for (const raw of content.split('\n')) {
    ...
    if (bulletItem || numberedItem) {
      const prefix = numberedItem ? `${numberedItem[1]}. ` : '• ';
      const itemText = numberedItem ? numberedItem[2] : bulletItem[1];
      paragraphs.push(new docx.Paragraph({
        children: [new docx.TextRun({ text: prefix, size: 20, font: 'Arial' }), ...parseInlineRuns(itemText, 20, docx)],
        indent: { left: 360 },
        spacing: { after: 80 },
      })); continue;
    }
    ...
    // For allday types: add space after colon when label pattern has no space (e.g. "Datum:Maandag")
    const paraText = (isAlldayType && !line.startsWith('*') && !line.startsWith('#') && !line.startsWith('|'))
      ? line.replace(/^([A-Za-z][^:\n*]*):([^\s:/])/, '$1: $2')
      : line;
    paragraphs.push(new docx.Paragraph({ children: parseInlineRuns(paraText, 20, docx), spacing: { after: 120 } }));
  }
  ...
  return docx.Packer.toBlob(wordDoc);
}
...
export async function downloadWordDoc(content, title, logos = {}, extras = null, filename = null, outputType = null, client = null, project = null) {
  if (!content?.trim()) return;
  const blob = await buildWordBlob(content, title, logos, extras, outputType, client, project);
  if (!blob) return;
  const wordMainTitle = buildDocTypeLabel(outputType) || title?.replace(/\[[A-Z][A-Z\s]*(:[^\]]*)?]/g, '').trim() || '';
