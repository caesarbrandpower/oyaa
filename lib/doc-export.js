...
export async function fetchImageAsBase64(url) {
  ...
  try {
    ...
    // Fallback: try direct fetch if proxy failed (works for public CORS-enabled hosts like Supabase storage)
    if (!res.ok && isExternalUrl(url)) res = await fetch(url);
...
export async function fetchImageAsBuffer(url) {
  ...
  try {
    const fetchUrl = isExternalUrl(url) ? `/api/proxy-logo?url=${encodeURIComponent(url)}` : url;
    let res = await fetch(fetchUrl);
    // Fallback: try direct fetch if proxy failed
    if (!res.ok && isExternalUrl(url)) res = await fetch(url);
    if (!res.ok) return null;
...
export function buildFilename(outputTypeLabel, client, project, extension) {
  ...
  const parts = [outputTypeLabel, client, project].filter(Boolean).map(sanitizePart).filter(Boolean);
...
function buildDocTypeLabel(outputType) {
  switch (outputType) {
    ...
    case 'account-to-pm': return 'BRIEFING ACCOUNT NAAR PM';
    case 'account-to-creation': return 'BRIEFING ACCOUNT NAAR CREATIE';
...
export async function downloadPdfDoc(content, title, logos = {}, extras = null, filename = null, outputType = null, client = null, project = null) {
  ...
  checkPage(wrappedTitle.length * 8 + 16);
  doc.text(wrappedTitle, margin, y);
  y += wrappedTitle.length * 8;

  if (pdfTypeLabel && pdfCpLabel) {
...
export async function downloadWordDoc(content, title, logos = {}, extras = null, filename = null, outputType = null, client = null, project = null) {
  ...
  // ── Logo header (dark background, chase left — client right via tab stop) ──
  if (logos.chaseBuffer || logos.clientBuffer) {
    const headerChildren = [];
    if (logos.chaseBuffer) {
      try {
        headerChildren.push(new docx.ImageRun({ data: logos.chaseBuffer, transformation: { width: 120, height: 40 }, type: 'png' }));
      } catch { /* skip unsupported format */ }
    }
    if (logos.clientBuffer) {
      headerChildren.push(new docx.TextRun({ text: '\t' }));
      try {
        headerChildren.push(new docx.ImageRun({ data: logos.clientBuffer, transformation: { width: 120, height: 40 }, type: 'png' }));
      } catch { /* skip */ }
    }
    paragraphs.push(new docx.Paragraph({
      shading: { fill: '111111', color: '111111', type: docx.ShadingType.SOLID },
      spacing: { before: 160, after: 160 },
      rightTabStop: 8800,
      children: headerChildren.length > 0 ? headerChildren : [new docx.TextRun({ text: ' ' })],
    }));
  ...
  paragraphs.push(new docx.Paragraph({
    children: [new docx.TextRun({ text: wordMainTitle, bold: true, size: 28, font: 'Arial' })],
    spacing: { after: wordTypeLabel && wordCpLabel ? 80 : 320 },
    border: wordTypeLabel && wordCpLabel ? undefined : { bottom: { color: 'DDDDDD', space: 1, style: docx.BorderStyle.SINGLE, size: 6 } },
  }));
  if (wordTypeLabel && wordCpLabel) {
    paragraphs.push(new docx.Paragraph({
      children: [new docx.TextRun({ text: wordCpLabel, size: 22, font: 'Arial', color: '888888' })],
      spacing: { after: 320 },
...
export async function shareDocument(content, title, client = null, extras = null, outputType = null) {
  ...
  const res = await fetch('/api/share-document', {
    ...
    body: JSON.stringify({ content: shareContent, outputType: outputType ?? null, title, client }),
  });
