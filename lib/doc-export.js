...
// Transforms the raw numbered-list Projectfase block to a single plain text line
...
function renderPhaseCompact(content) {
  ...
  const line = `Fase ${phaseNum} — ${name}${nextNum ? ` → Fase ${nextNum} — ${nextName}` : ''}`;
  return content.replace(m[0], `**Projectfase**\n\n${line}\n`);
}
