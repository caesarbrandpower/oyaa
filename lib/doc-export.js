...
export async function fetchImageAsBase64(url) {
  ...
  try {
    ...
    if (isSupabasePublic) {
      res = await fetch(url);
      // Fallback naar proxy als directe fetch mislukt (bijv. bucket privé geworden)
      if (!res.ok) res = await fetch(`/api/proxy-logo?url=${encodeURIComponent(url)}`);
    } else if (isExternalUrl(url)) {
      res = await fetch(`/api/proxy-logo?url=${encodeURIComponent(url)}`);
...
export function buildFilename(outputType, client, project, extension) {
  ...
  const parts = [typeLabel, client, project].filter(Boolean).map(sanitizePart).filter(Boolean);
...
export async function downloadWordDoc(content, title, logos = {}, extras = null, filename = null, outputType = null, client = null, project = null) {
  ...
  if (logos.chaseBuffer || logos.clientBuffer) {
    try {
      const headerBuf = await createHeaderImageBuffer(logos.chaseBuffer, logos.clientBuffer);
      if (headerBuf) {
