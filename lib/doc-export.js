    ...
    if (!segs.some(s => s.bold)) {
      ...
      if (text.startsWith('|') && text.endsWith('|')) {
        ...
        if (cells.length >= 2) {
          ...
          const col1Lines = doc.splitTextToSize(cells[0], col1W);
          doc.setFont('helvetica', 'normal');
          const col2Lines = doc.splitTextToSize(cells.slice(1).join(' '), col2W);
          const rowH = Math.max(col1Lines.length, col2Lines.length) * 5 + 3;
          checkPage(rowH);
          doc.setFont('helvetica', 'bold');
          doc.text(col1Lines, margin, y);
          doc.setFont('helvetica', 'normal');
          doc.text(col2Lines, col2X, y);
          y += rowH;
        } else if (cells.length === 1) {
          doc.setFont('helvetica', 'normal');
          const wrapped = doc.splitTextToSize(cells[0], maxWidth);
