...
export async function extractFileText(buffer, filename) {
  ...
  if (ext === 'pdf') {
    ...
    const raw = pageTexts.filter(Boolean).join('\n\n');
    // Design-PDFs (InDesign, Illustrator) plaatsen elke letter als apart tekst-item,
    // waardoor unpdf "E I N D E V A L U A T I E" produceert. Detecteer en herstel.
    return raw.replace(/(?<!\w)([A-Z0-9](?: [A-Z0-9]){3,})(?!\w)/g, (m) => m.replace(/ /g, ''));
  }
