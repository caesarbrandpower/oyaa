...
// Berichten die beginnen met een vraagwoord of eindigen op "?" zijn nooit schrijfverzoeken.
const QUESTION_START_RE = /^(wat|welke?|waar|wanneer|hoeveel|hoe|wie|is|zijn|heeft|hebben|kan|mag|moet)\b/i;

export function isQuestion(message) {
  const t = message.trim();
  return QUESTION_START_RE.test(t) || t.endsWith('?');
}
 ...
 */
export async function parseWriteIntent(message, locationNames, anthropicClient, recentMessages = []) {
...
${recentMessages.length > 0 ? `Recente gesprekscontext (gebruik dit als het huidige bericht een locatienaam mist):\n${recentMessages.map(m => `Gebruiker: ${m}`).join('\n')}\n\n` : ''}Huidig bericht: "${message}"
...
Als de locatienaam niet in de lijst staat of het bericht geen schrijf-intentie bevat: {"locatie_naam": null, "veld": null, "nieuwe_waarde": null, "modus": null}`,
  ...
  // Lege nieuwe waarde is nooit een geldig schrijfverzoek — vangt vragen op die
  // een veld noemen maar geen waarde geven ("Wat is het adres van X?").
  if (parsed.modus !== 'verwijder' && !parsed.nieuwe_waarde?.trim()) return null;
