...
export async function addPhotoPlaceholders(pptxBuffer, data) {
  ...
  for (const photo of PHOTOS) {
    ...
    const rawFotoData = data[field] ?? null;

    // URL → fetch en omzetten naar data-URI
    let base64Data;
    if (rawFotoData && rawFotoData.startsWith('http')) {
      try {
        const resp = await fetch(rawFotoData);
        const ct   = resp.headers.get('content-type') || 'image/jpeg';
        const buf  = Buffer.from(await resp.arrayBuffer());
        base64Data = `data:${ct};base64,${buf.toString('base64')}`;
      } catch { base64Data = null; }
    } else {
      base64Data = rawFotoData;
    }
