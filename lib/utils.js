const CANONICAL_CLIENTS = {
  ...
  'cocacola': 'Coca-Cola',
  'coca-cola': 'Coca-Cola',
  'coca colaa': 'Coca-Cola',
  'cocacollaa': 'Coca-Cola',
  'coca-colaa': 'Coca-Cola',
  'coce cola': 'Coca-Cola',
  'coca col': 'Coca-Cola',
  'coke': 'Coca-Cola',
...
export function normalizeClientName(name, existingClients = []) {
  ...
  const canonicalKey = trimmed.toLowerCase().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, ' ').trim();
  if (CANONICAL_CLIENTS[canonicalKey]) return CANONICAL_CLIENTS[canonicalKey];
