...
export async function buildLocationNameContext(tenant, supabase) {
  ...
  return `## Locatieoverzicht (${tenantLabel})\nOnderstaand alle locaties op naam. Gebruik de tool search_locations voor bereik, kosten, CPM en vergelijkingen — de namen hieronder bevatten geen cijfers. Gebruik de tool ook wanneer de gebruiker een overzicht vraagt mét bereik of kosten. Roep de tool direct aan zonder eerst om verduidelijking te vragen. Bij vergelijkingsvragen: gebruik vergelijkbaar_met met een deel van de naam (bijv. "Utrecht Centraal" matcht automatisch alle varianten). CPM = kosten per 1.000 bereik; lager is efficiënter.\n\n${index}`;
}
