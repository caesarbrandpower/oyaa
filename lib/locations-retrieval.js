...
/**
 * Stripped index zonder bereik/kosten/cpm — alleen voor contexten waar Layer 3
 * (vergelijkingstabel) actief is. Geeft het model een volledig beeld van welke
 * locaties bestaan en waar ze liggen, maar verwijdert alle numerieke kolommen
 * waarop het anders zelf zou kunnen vergelijken of rangschikken.
 */
...
- Zonder bereikwaarde (${zonderBereik.length}): ${zonderBereikLijst}`;
    ...
    // Als Layer 3 actief is: vervang de volledige index door een naam-only versie
    // zonder bereik/kosten/cpm. Het model weet dan nog steeds welke locaties bestaan
    // (ook voor gemengde vragen als "alternatieven voor X én locaties in Y"), maar
    // kan de numerieke kolommen niet gebruiken voor een eigen vergelijking.
    const index = layer3Block ? buildLocationNameIndex(locs) : buildLocationIndex(locs);
    const indexNote = layer3Block
      ? '\nLOCATIEOVERZICHT: uitsluitend namen, stad en channel. Bereik/kosten/cpm zijn weggelaten — gebruik de Vergelijkingstabel verderop voor alle kostenvergelijkingen.'
      : '';

    let block = `## Locatiedatabase (${tenantLabel})\nBereikcijfers zijn daggemiddelden op basis van jaarschattingen — gebruik ze als indicatie, niet als meting.\nCPM = kosten per 1.000 bereik; lager is efficiënter. CPM is alleen berekend voor huurprijzen.\nKosten: een vast bedrag per activatie. Er is geen periode in de data — voeg nooit een tijdseenheid toe.${indexNote}\n\n${statistieken}\n\n${index}`;
