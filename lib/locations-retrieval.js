...
function formatLocationTable(locs) {
  ...
  const rows = locs.map(l => {
    ...
    // Detail-velden: worden ingelezen door SELECT maar mogen het model bereiken.
    const details = [];
    if (l.adres)         details.push(`  adres: ${l.adres}`);
    if (l.omschrijving)  details.push(`  omschrijving: ${l.omschrijving}`);
    if (l.parkeren)      details.push(`  parkeren: ${l.parkeren}`);
    if (l.laden_lossen)  details.push(`  laden_lossen: ${l.laden_lossen}`);
    ...
    if (l.bijzonderheden) details.push(`  bijzonderheden: ${l.bijzonderheden}`);
