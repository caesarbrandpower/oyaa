...
export async function buildDatabaseContext(tenant, supabase, userMessage) {
  ...
  if (locationsOn && locResult.data?.length > 0) {
    ...
    let block = `## Locatiedatabase (${tenantLabel})\nBereikcijfers zijn daggemiddelden op basis van jaarschattingen — gebruik ze als indicatie, niet als meting. CPM is alleen berekend voor huurprijzen.\nKosten: het veld "kosten" is een vast bedrag per activatie. Er is geen periode in de data. Voeg nooit een tijdseenheid toe (geen "/week", "/dag", "/maand" of andere periode). Toon de waarde letterlijk zoals die in de tabel staat.\nTellen: als je een aantal locaties noemt, tel ze zelf uit de tabel — doe geen schatting.\nVergelijken: bij een vergelijkingsvraag eerst de kandidaten vaststellen op basis van de data, dan pas een conclusie trekken. Zeg nooit "er zijn geen goedkopere/betere opties" als die er in dezelfde tabel staan.\n\n${index}`;
