export const LOCATION_TOOL_SCHEMA = {
  ...
  description: 'Zoek locaties op in de database. Gebruik dit voor alle vragen over bereik, kosten, CPM, vergelijkingen of doelgroepen. Geef altijd een tool-call als de gebruiker vraagt naar kosten, bereik, alternatieven, vergelijkbare locaties, of welke locaties een bepaalde doelgroep bereiken.',
  input_schema: {
    type: 'object',
    properties: {
      naam_bevat: {
      ...
      inclVervallen: {
        type: 'boolean',
        description: 'Wanneer true: vervallen locaties worden meegenomen in de resultaten. Gebruik dit bij vragen als "welke locaties gebruiken we niet meer?" of "geef vervallen locaties". Standaard (false) worden vervallen locaties uitgesloten.',
      },
    },
  },
};
...
export async function executeLocationTool(params, supabase, tenantId) {
  try {
    ...
    if (!inclVervallen) results = results.filter(l => l.status !== 'vervallen');
