export const LOCATION_TOOL_SCHEMA = {
  ...
  description: 'Zoek locaties op in de database. Gebruik dit voor alle vragen over bereik, kosten, CPM of vergelijkingen. Geef altijd een tool-call als de gebruiker vraagt naar kosten, bereik, alternatieven of vergelijkbare locaties.',
  input_schema: {
    type: 'object',
    properties: {
      naam_bevat: {
      ...
      vergelijkbaar_met: {
        type: 'string',
        description: 'Naam van een locatie (exact, hoofdletterongevoelig). De server berekent bereik-drempels (factor 3 rondom het gemiddelde bereik van de referentielocatie) en geeft uitsluitend locaties binnen dat bereikvenster terug, gesorteerd op CPM oplopend. Gebruik dit voor vergelijkingsvragen.',
      },
      limiet: {
        type: 'integer',
...
export async function executeLocationTool(params, supabase, tenantId) {
  try {
    ...
    if (vergelijkbaar_met) {
      ...
      if (refs.length > 0) {
        ...
        if (refValues.length > 0) {
          ...
          // Prefix de output met context over de referentie
          const refInfo = `Referentie: ${refs.map(r => r.naam).join(', ')} — gemiddeld bereik ${refAvg.toLocaleString('nl-NL')}/dag\nBereikvenster: ${min.toLocaleString('nl-NL')}–${max.toLocaleString('nl-NL')} (factor 3)\n\n`;
          // Sorteer altijd op CPM bij vergelijkbaar_met
          results = sortLocations(results, 'cpm_asc');
          return refInfo + formatLocationTable(results.slice(0, limiet));
        }
      }
      return `Referentielocatie "${vergelijkbaar_met}" niet gevonden.`;
    }
    ...
    if (!results.length) return 'Geen locaties gevonden met deze filters.';

    return formatLocationTable(results.slice(0, limiet));
  } catch {
    return 'Tool-uitvoering mislukt.';
  }
}
...
function formatLocationTable(locs) {
  ...
  const header = 'naam | bereik/dag | kosten | cpm | status';
...
export async function buildLocationNameContext(tenant, supabase) {
  ...
  return `## Locatieoverzicht (${tenantLabel})\nOnderstaand alle locaties op naam. Gebruik de tool search_locations voor bereik, kosten, CPM en vergelijkingen — de namen hieronder bevatten geen cijfers. CPM = kosten per 1.000 bereik; lager is efficiënter. Bij vergelijkingsvragen: gebruik vergelijkbaar_met met de exacte locatienaam.\n\n${index}`;
}
