...
export async function executeLocationTool(params, supabase, tenantId) {
  try {
    ...
    if (vergelijkbaar_met) {
      ...
      if (refs.length > 0) {
        ...
        if (refValues.length > 0) {
          ...
          return {
            text: refInfo + formatLocationTable(sliced, sliced.length <= 3),
    ...
    const sliced = results.slice(0, limiet);
    // Detail-velden (adres, bijzonderheden, etc.) alleen bij ≤3 resultaten.
    // Bij 4+ resultaten: tabel-overzicht; het model vermeldt in het antwoord
    // dat details per locatie op te vragen zijn.
    return {
      text: formatLocationTable(sliced, sliced.length <= 3),
      locations: sliced.map(l => ({ naam: l.naam, stad: l.stad ?? null, channel: l.channel ?? null })),
    };
  } catch {
...
// Vervangt newlines in veldwaarden door een scheidingsteken zodat de
// tool-output per locatie op één regel blijft.
function singleLine(s) {
  return s.replace(/\r?\n+/g, ' · ');
}

function formatLocationTable(locs, includeDetails = true) {
  ...
  const rows = locs.map(l => {
    ...
    const row = [l.naam, bereik, kosten, cpm, l.status ?? 'onbekend'].join(' | ');

    if (!includeDetails) return row;

    // Detail-velden (alleen bij ≤3 resultaten): newlines worden platgeslagen
    // zodat elke regel precies één locatie-veld beschrijft.
    const details = [];
    if (l.adres)         details.push(`  adres: ${singleLine(l.adres)}`);
    if (l.omschrijving)  details.push(`  omschrijving: ${singleLine(l.omschrijving)}`);
    if (l.parkeren)      details.push(`  parkeren: ${singleLine(l.parkeren)}`);
    if (l.laden_lossen)  details.push(`  laden_lossen: ${singleLine(l.laden_lossen)}`);
    if (l.vergunning_status) {
      const vd = l.vergunning_vervaldatum ? ` (t/m ${l.vergunning_vervaldatum})` : '';
      details.push(`  vergunning: ${l.vergunning_status}${vd}`);
    }
    if (l.bijzonderheden) details.push(`  bijzonderheden: ${singleLine(l.bijzonderheden)}`);
    if (Array.isArray(l.doelgroep) && l.doelgroep.length) details.push(`  doelgroep: ${l.doelgroep.join(', ')}`);
    if (Array.isArray(l.leeftijdsgroep) && l.leeftijdsgroep.length) details.push(`  leeftijdsgroep: ${l.leeftijdsgroep.join(', ')}`);

    return details.length ? [row, ...details].join('\n') : row;
  });
