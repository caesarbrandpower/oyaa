...
- De dagentabel bevat rijen per dag (dag / samples / bezoekers). Tel ALLE dagrijen op om totaal_samples en totaal_bezoekers te berekenen. Gebruik geen enkel ander totaal.
- De locatietabel bevat doel en gerealiseerd PER LOCATIE. Gebruik deze uitsluitend voor het "gerealiseerd" veld per locatie. Gebruik het totaal van de locatietabel NOOIT als totaal_samples of totaal_bezoekers.
- Als de som van de dagrijen verschilt van een eventueel totaalregel in de export: gebruik altijd de som van de dagrijen.
- Controleer jezelf: som je dagrijen op en schrijf het resultaat in het JSON-veld "target" en "dagen". Als je een ander getal ziet in de JSON dat afwijkt van je eigen optelsom, is dat fout.
...
1. Een korte samenvatting in het Nederlands (maximaal drie zinnen)
2. Exact één \`\`\`json code-blok met de evaluatiedata — geen andere tekst erna
...
VERPLICHTE ZELFCHECK vóór je JSON genereert:
1. Tel de samples per dag handmatig op uit de dagentabel.
2. Vergelijk die som met het getal dat je wilt invullen in totaal_samples.
3. Als ze niet gelijk zijn, herstel dan totaal_samples naar de optelsom van de dagrijen — niet het totaal uit de locatietabel of een andere bron.
Schrijf de optelsom als gedachtengang in je redenering vóór de JSON, zodat je jezelf kunt controleren.`,
};
