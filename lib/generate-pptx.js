...
export async function buildPresentation(data) {
  ...
  function addLogo(slide, variant = 'main', x = 8.0, y = 5.1) {
    slide.addImage({
  ...
  function addFotoPlaceholder(slide, x, y, w, h, label = 'FOTO') {
    ...
    slide.addText(`[ ${label} ]`, {
      x, y, w, h,
      fontSize: 11, color: 'AAAAAA', fontFace: 'Tungsten Book',
      align: 'center', valign: 'middle', margin: 0,
    });
  }

  function addFoto(slide, fotoData, x, y, w, h, label) {
    if (fotoData) {
      slide.addImage({ data: fotoData, x, y, w, h, sizing: { type: 'cover', w, h } });
    } else {
      addFotoPlaceholder(slide, x, y, w, h, label);
    }
  }
  ...
  // SLIDE 1 — TITELSLIDE
  {
    ...
    if (data.foto_product) {
      s.addImage({ data: data.foto_product, x: 5.2, y: 0, w: 4.8, h: 5.625,
      ...
      s.addShape(pres.shapes.RECTANGLE, {
        ...
        fill: { color: '1A1E6A' }, line: { color: '1A1E6A', width: 0 },
      });
      s.addText('[ PRODUCTFOTO ]', {
        x: 5.5, y: 0, w: 4.5, h: 5.625,
        fontSize: 11, color: '3A4090', fontFace: 'Tungsten Book',
        align: 'center', valign: 'middle', margin: 0,
      });
    }
    s.addImage({ path: LOGO_DIAP, x: 0.7, y: 0.35, w: 2.5, h: 0.62,
      sizing: { type: 'contain', w: 2.5, h: 0.62 } });
    ...
    s.addText('Gegenereerd via Waybetter', {
      x: 0.7, y: 5.18, w: 3, h: 0.25,
      fontSize: 8, color: '3A4090', fontFace: 'Tungsten Book', margin: 0,
    });
    addPageNum(s, 1);
  }
  ...
  // SLIDE 2 — KERNRESULTATEN
  {
    ...
    cards.forEach((card, i) => {
      ...
      s.addText(card.label, {
        ...
        fontFace: 'Tungsten Bold', margin: 0,
      });
  ...
  // SLIDE 5 — VERLOOP VAN DE ACTIE
  {
    ...
    data.verloop.forEach((item, i) => {
      ...
      s.addText(item.label.toUpperCase(), {
        ...
        fontSize: 9, bold: true, charSpacing: 2, color: C.navy,
        fontFace: 'Tungsten Bold', margin: 0,
      });
      s.addText(item.tekst, {
        x: 0.65, y: y + 0.38, w: 4.95, h: 0.65,
        fontSize: 11, color: C.darkgrey, fontFace: 'Tungsten Book',
        valign: 'top', margin: 0,
      });
    });
    addFoto(s, data.foto_actie1, 6.1, 1.0, 3.65, 3.85, 'ACTIEFOTO');
    addLogo(s);
    addPageNum(s, 5);
  }
  ...
  // SLIDE 6 — TERUGBLIK
  {
    ...
    cols.forEach((col, i) => {
      ...
      s.addText(col.label, {
        ...
        fontSize: 12, bold: true, charSpacing: 2, color: col.labelColor,
        fontFace: 'Tungsten Bold', margin: 0,
      });
      ...
      s.addText(items, {
        ...
        fontSize: 13, fontFace: 'Tungsten Book', valign: 'top',
  ...
  // SLIDE 7 — ACTIEPUNTEN
  {
    ...
    s.addText('VOLGENDE EDITIE', {
      ...
      fontSize: 13, color: C.greyblue, fontFace: 'Tungsten Book', margin: 0,
    });
    data.actiepunten.forEach((actie, i) => {
      ...
      s.addText(actie, {
        ...
        fontSize: 13, color: C.navy, fontFace: 'Tungsten Book', valign: 'middle', margin: 0,
      });
    });
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  ...
  // SLIDE 8 — EINDSLIDE
  {
    ...
    if (data.foto_actie2) {
      s.addImage({ data: data.foto_actie2, x: 0.5, y: 0, w: 9.5, h: 5.625,
        sizing: { type: 'cover', w: 9.5, h: 5.625 } });
      s.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: 0, w: 9.5, h: 5.625,
        fill: { color: C.navy, transparency: 55 }, line: { color: C.navy, width: 0 },
      });
    } else {
      s.addShape(pres.shapes.RECTANGLE, {
        x: 0.5, y: 0, w: 9.5, h: 5.625,
      ...
      s.addText('[ ACTIEFOTO ]', {
        x: 0.5, y: 0, w: 9.5, h: 5.625,
        fontSize: 13, color: '3A4090', fontFace: 'Tungsten Book',
        align: 'center', valign: 'middle', margin: 0,
      });
    }
    s.addText(data.campagne.toUpperCase(), {
    ...
    s.addText(`${data.klant}  ·  ${data.datum}`, {
      ...
      fontSize: 12, color: C.greyblue, fontFace: 'Tungsten Book', margin: 0,
    });
    s.addImage({ path: LOGO_DIAP, x: 8.0, y: 5.05, w: 1.8, h: 0.44,
      sizing: { type: 'contain', w: 1.8, h: 0.44 } });
    addPageNum(s, 8);
  }
