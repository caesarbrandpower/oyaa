...
export async function buildPresentation(data, theme) {
  const { colors: C, logos, fonts } = theme;
  ...
  function addLogo(slide, variant = 'main', x = 8.82, y = 5.08) {
    slide.addImage({
      path: variant === 'diap' ? logos.diap : logos.main,
      x, y, w: 1.06, h: 0.44,
      sizing: { type: 'contain', w: 1.06, h: 0.44 },
    });
  }
  ...
  function slideTitle(slide, text) {
    slide.addText(text, {
      ...
      fontFace: fonts.bold, align: 'left', margin: 0,
    });
  }
  ...
  // SLIDE 1 — TITELSLIDE
  {
    ...
    s.addImage({ path: logos.diap, x: 0.7, y: 0.35, w: 1.5, h: 0.62,
      sizing: { type: 'contain', w: 1.5, h: 0.62 } });
    s.addText(data.campagne, {
      x: 0.7, y: 1.35, w: 4.5, h: 1.4,
      fontSize: 30, bold: true, color: C.white,
      fontFace: fonts.bold, align: 'left', margin: 0,
    });
    s.addText([
      { text: data.klant, options: { bold: true, color: C.accent } },
      { text: `  ·  ${data.datum}`, options: { color: C.greyblue } },
    ], {
      x: 0.7, y: 2.88, w: 4.5, h: 0.45,
      fontSize: 15, fontFace: fonts.book, align: 'left', margin: 0,
    });
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: 0.7, y: 3.5, w: 2.0, h: 0.4,
      fill: { color: C.accent }, line: { color: C.accent, width: 0 }, rectRadius: 0.05,
    });
    s.addText('Campagne evaluatie', {
      x: 0.7, y: 3.5, w: 2.0, h: 0.4,
      fontSize: 13, bold: true, charSpacing: 1, color: C.navy,
      fontFace: fonts.bold, align: 'center', valign: 'middle', margin: 0,
    });
    addPageNum(s, 1);
  }
  ...
  // SLIDE 2 — KERNRESULTATEN
  {
    ...
    cards.forEach((card, i) => {
      ...
      s.addText(card.label, {
        ...
        color: card.highlight ? C.accent : C.greyblue,
        fontFace: fonts.book, margin: 0,
      });
      s.addText(card.value, {
        ...
        fontFace: fonts.book, align: 'center', valign: 'middle', margin: 0,
      });
      s.addText(card.sub, {
        x: x + 0.05, y: 2.88, w: 2.08, h: 0.32,
        fontSize: 12, color: C.greyblue,
        fontFace: fonts.book, align: 'center', margin: 0,
      });
    });
    s.addText(`${data.dagen.length} dagen  ·  ${data.locaties.length} locaties  ·  ${data.periode}`, {
      x: 0.4, y: 4.0, w: 9, h: 0.35,
      fontSize: 12, color: C.darkgrey, fontFace: fonts.book, margin: 0,
    });
    addLogo(s);
    addPageNum(s, 2);
  }
  ...
  // SLIDE 3 — AANTALLEN PER DAG
  {
    ...
    s.addTable(tabelRows, {
      ...
      fontSize: 12, fontFace: fonts.book, rowH: 0.46,
    });
    addLogo(s);
    addPageNum(s, 3);
  }
  ...
  // SLIDE 4 — RESULTATEN PER LOCATIE
  {
    ...
    s.addTable(rows, {
      ...
      fontSize: 12, fontFace: fonts.book, rowH: 0.48,
    });
    s.addChart(pres.charts.BAR, [
  ...
  // SLIDE 5 — VERLOOP VAN DE ACTIE
  {
    ...
    data.verloop.forEach((item, i) => {
      ...
      s.addText(item.tekst, {
        ...
        fontSize: 10, color: C.darkgrey, fontFace: fonts.body,
        valign: 'top', margin: 0,
      });
    });
    addLogo(s);
  ...
  // SLIDE 6 — TERUGBLIK
  {
    ...
    const cols = [
      { label: 'Wat ging goed', items: data.goedGegaan,     bgCard: C.navy,  textColor: C.white, labelColor: C.accent },
    ...
    cols.forEach((col, i) => {
      ...
      s.addText(items, {
        ...
        fontSize: 12, fontFace: fonts.body, valign: 'top',
  ...
  // SLIDE 7 — ACTIEPUNTEN
  {
    ...
    s.addText('Volgende editie', {
      ...
      fontSize: 14, bold: false, color: C.greyblue, fontFace: fonts.book, margin: 0,
    });
    data.actiepunten.forEach((actie, i) => {
      ...
      s.addText(actie, {
        ...
        fontSize: 12, color: C.navy, fontFace: fonts.body, valign: 'middle', margin: 0,
      });
    });
    addLogo(s);
    addPageNum(s, 7);
  ...
  // SLIDE 8 — EINDSLIDE
  {
    ...
    s.addText(data.campagne, {
      ...
      fontFace: fonts.book, align: 'left', margin: 0,
    });
    s.addText(`${data.klant}  ·  ${data.datum}`, {
      x: 0.8, y: 4.38, w: 5.5, h: 0.35,
      fontSize: 12, color: C.greyblue, fontFace: fonts.book, margin: 0,
    });
    s.addImage({ path: logos.diap, x: 8.82, y: 5.08, w: 1.06, h: 0.44,
      sizing: { type: 'contain', w: 1.06, h: 0.44 } });
    addPageNum(s, 8);
  }
