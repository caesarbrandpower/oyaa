...
export function useAudioTranscription({ onTranscript, onStatus, onError }) {
  ...
  const [transcribeProgress, setTranscribeProgress] = useState(null); // { current, total } | null
  const [transcribingElapsed, setTranscribingElapsed] = useState(0);
  ...
  useEffect(() => {
    if (transcribing) {
      setTranscribingElapsed(0);
      transcribingTimerRef.current = setInterval(() => {
        setTranscribingElapsed((prev) => prev + 1);
      }, 1000);
    } else {
      clearInterval(transcribingTimerRef.current);
      setTranscribingElapsed(0);
    }
    return () => clearInterval(transcribingTimerRef.current);
  }, [transcribing]);
  ...
  const transcribeFile = useCallback(async (file, isLiveRecording = false) => {
    ...
    try {
      if (isLiveRecording) {
        ...
        setTranscribeProgress({ current: 0, total: 1, filename: file.name });
        ...
        try {
          ...
          if (cancelRef.current) {
            onStatus?.('Transcriptie geannuleerd.');
          } else {
        ...
        if (data.error) {
          ...
          setTranscribeProgress({ current: 1, total: 1, filename: file.name });
      ...
      setTranscribeProgress({ current: 0, total: totalChunks, filename: file.name });
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        setTranscribeProgress({ current: i + 1, total: totalChunks, filename: file.name });
      }
